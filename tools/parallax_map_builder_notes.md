...
## Prefab storage expectations

`tools/convert-map-layout.mjs` automatically looks for referenced prefabs under
`prefabs/structures/` when converting a layout. Place each builder-exported
structure JSON in that directory (or provide a custom path via
`--prefab-dir`) using a filename that matches the `prefabId` (for example,
`tree.json` or `plants/oak.prefab.json`). Prefabs that cannot be resolved are
emitted as warnings so missing assets never halt the conversion run.
