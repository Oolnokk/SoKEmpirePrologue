...
function resolveFallbackGameplayPathTarget(options) {
  const {
    area,
    lookAtOffsetsWorld,
    gridCenterX,
  ...
  // Preferred fallback: derive from visualsmap gameplayPath even when no ground tiles are placed on it
  if (gameplayPath?.start && gameplayPath?.end && Number.isFinite(cellSize)) {
    const startWorld = gridToWorld(gameplayPath.start.row, gameplayPath.start.col, rows, cols, cellSize, pathYawRad, alignWorldToPath);
    const endWorld = gridToWorld(gameplayPath.end.row, gameplayPath.end.col, rows, cols, cellSize, pathYawRad, alignWorldToPath);

    return {
      x: (startWorld.x + endWorld.x) / 2,
      y: ((startWorld.y + endWorld.y) / 2) + lookAtOffsetsWorld.y,
      z: ((startWorld.z + endWorld.z) / 2) + lookAtOffsetsWorld.z,
      source: 'visualsmapGameplayPath',
    };
  }

  // Last resort: average gameplay ground path from gameplay map; keep center/plane alignment
  ...
  try {
    ...
    const gameplayPathTarget = (() => {
      if (!pathGroundSamples.length) {
        ...
        return { x: gridCenterX, y: lookAtOffsetsWorld.y, z: gridCenterZ + lookAtOffsetsWorld.z, source: 'gridCenterFallback' };
      }
    ...
    const targetSource = gameplayPathTarget?.source || 'unknown';
    console.log(`[visualsmapLoader] Gameplay path target source: ${targetSource}`);
    if (fallbackGameplayPathTarget) {
      console.log(`[visualsmapLoader] Gameplay path fallback target from ${fallbackGameplayPathTarget.source}: (${fallbackGameplayPathTarget.x.toFixed(1)}, ${fallbackGameplayPathTarget.y.toFixed(1)}, ${fallbackGameplayPathTarget.z.toFixed(1)})`);
    }
