// animator.js — restore basic idle/walk posing with v19-style degrees→radians mapping
// Drives jointAngles (radians) for torso/limbs. Attacks can override by writing
// window.GAME.poseOverride = { fighterId:'player'|'npc', pose:{...deg...}, durMs?:number }
// and we will blend to that for durMs, then release.
...
function damp(current, target, lambda, dt){ // exponential smoothing
...
function computeWalkPose(F, C){
  ...
  const speed = Math.abs(F.vel?.x || 0);
  ...
  // shapes between A and B using sin
  const s = (Math.sin(F.walk.phase)+1)/2; // 0..1
  const A = W.poses?.A || {};
  const B = W.poses?.B || {};
  const pose = Object.assign({}, pickBase(C));
  // interpolate hips/knees (deg)
  pose.lHip = lerp(A.lHip||0, B.lHip||0, s);
  pose.lKnee= lerp(A.lKnee||0,B.lKnee||0,s);
  pose.rHip = lerp(A.rHip||0, B.rHip||0, s);
  pose.rKnee= lerp(A.rKnee||0,B.rKnee||0,s);
  pose.torso= lerp(A.torso||0,B.torso||0,s);
  // leave arms near stance for now
  const base = pickBase(C);
  pose.lShoulder = base.lShoulder; pose.lElbow = base.lElbow;
  pose.rShoulder = base.rShoulder; pose.rElbow = base.rElbow;
  pose._active = on;
  return pose;
}
...
export function updatePoses(){
  ...
  for (const id of ['player','npc']){
    const F = G.FIGHTERS[id]; if(!F) continue;
    ensureAnimState(F);
    ...
    const target = degToRadPose(targetDeg);

    // 2) damp angles toward target
    const lambda = 10; // smoothing factor
    for(const k of ANG_KEYS){
      const cur = F.jointAngles[k] ?? 0;
      const tar = target[k] ?? cur;
      F.jointAngles[k] = damp(cur, tar, lambda, F.anim.dt);
    }
  }
}

// Optional external API to push a timed override (deg-based pose)
export function pushPoseOverride(fighterId, poseDeg, durMs=300){
  const G = window.GAME || {};
  const F = G.FIGHTERS?.[fighterId]; if(!F) return;
  ensureAnimState(F);
  F.anim.override = { pose: poseDeg, until: performance.now()/1000 + (durMs/1000) };
}
