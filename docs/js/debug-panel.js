...
import {
  toggleSegPosStandardBasis,
  toggleWithAXStandardMapping,
  toggleNegateLowerJoints,
  toggleGlobalRotation90,
  toggleAltSegPos,
  revertAllFixes,
  getToggleStates,
  testOrdering
} from './runtime-fixes.js?v=1';
...
export function updateDebugPanel() {
  ...
  // Update runtime fixes section
  updateRuntimeFixesSection();
}
...
// ========================================================================
// Runtime Fixes Section
// ========================================================================

/**
 * Initialize runtime fixes section in the debug panel
 * This section provides toggles for experimental runtime patches
 */
function initRuntimeFixesSection() {
  const container = $$('#debugRuntimeFixes');
  if (!container) return;
  
  let html = '<div class="debug-section-header">';
  html += '<div class="debug-section-title">⚠️ Runtime Fix Toggles (Experimental)</div>';
  html += '<button type="button" id="btnRevertAllFixes" class="debug-revert-btn">Revert All</button>';
  html += '</div>';
  
  html += '<div class="debug-fixes-note">';
  html += 'These are temporary runtime-only experiments for diagnosing 90°/sign/basis issues. ';
  html += 'Changes affect player fighter only and are fully reversible. No source files are modified.';
  html += '</div>';
  
  // Ordering check status
  html += '<div id="orderingStatus" class="debug-ordering-status">Checking...</div>';
  
  // Toggle checkboxes
  const toggles = [
    {
      id: 'segPosStandard',
      label: 'Use standard segPos basis (cos/sin)',
      description: 'Changes segPos to use standard mathematical basis instead of "up" convention'
    },
    {
      id: 'withAXStandard',
      label: 'Use standard withAX mapping (cos/sin)',
      description: 'Changes withAX to use standard cos/sin rotation matrix'
    },
    {
      id: 'negateLower',
      label: 'Negate lower joints (elbows/knees)',
      description: 'Applies runtime negation to lower joint angles to fix sign issues'
    },
    {
      id: 'global90',
      label: '+90° global joint rotation',
      description: 'Adds +90° to all joint angles (useful if entire skeleton is rotated)'
    },
    {
      id: 'altSegPos',
      label: 'Alternative segPos (-cos, sin)',
      description: 'Alternative basis for segPos that might fix specific orientation issues'
    }
  ];
  
  html += '<div class="debug-toggles-grid">';
  
  for (const toggle of toggles) {
    html += '<div class="debug-toggle-item">';
    html += '<label class="debug-toggle-label">';
    html += `<input type="checkbox" id="toggle_${toggle.id}" class="debug-toggle-checkbox" data-toggle="${toggle.id}">`;
    html += `<span class="debug-toggle-text">${toggle.label}</span>`;
    html += `<span id="status_${toggle.id}" class="debug-toggle-status">Inactive</span>`;
    html += '</label>';
    html += `<div class="debug-toggle-description">${toggle.description}</div>`;
    html += '</div>';
  }
  ...
  // Attach event listeners
  const revertBtn = $$('#btnRevertAllFixes');
  if (revertBtn) {
    revertBtn.addEventListener('click', handleRevertAll);
  }
  
  // Toggle checkboxes
  for (const toggle of toggles) {
    const checkbox = $$(`#toggle_${toggle.id}`);
    if (checkbox) {
...
/**
 * Update runtime fixes section with current states
 */
function updateRuntimeFixesSection() {
  ...
  for (const [key, value] of Object.entries(states)) {
    const checkbox = $$(`#toggle_${key}`);
    const status = $$(`#status_${key}`);
    ...
    if (status) {
      status.textContent = value ? 'Active' : 'Inactive';
      status.className = `debug-toggle-status ${value ? 'status-active' : 'status-inactive'}`;
    }
  }
  
  // Update ordering check
...
function handleToggleChange(event) {
  ...
  console.log(`[debug-panel] Toggle ${toggleId} -> ${enabled}`);
  
  // Call the appropriate toggle function
  switch (toggleId) {
    case 'segPosStandard':
      toggleSegPosStandardBasis(enabled);
      break;
    case 'withAXStandard':
      toggleWithAXStandardMapping(enabled);
      break;
    case 'negateLower':
      toggleNegateLowerJoints(enabled);
      break;
    case 'global90':
      toggleGlobalRotation90(enabled);
      break;
    case 'altSegPos':
      toggleAltSegPos(enabled);
      break;
  }
  
  // Update ordering check immediately
  updateOrderingStatus();
}
...
function handleRevertAll() {
  console.log('[debug-panel] Reverting all runtime fixes');
  revertAllFixes();
  updateRuntimeFixesSection();
}

/**
 * Update ordering check status display
 */
function updateOrderingStatus() {
  const statusEl = $$('#orderingStatus');
  if (!statusEl) return;
  
  const result = testOrdering();
  
  let html = '<div class="ordering-result">';
  html += `<span class="ordering-label">Skeleton Ordering Check:</span>`;
  html += `<span class="ordering-status ${result.pass ? 'status-pass' : 'status-fail'}">${result.pass ? 'PASS' : 'FAIL'}</span>`;
  html += `<span class="ordering-penalty">Penalty: ${result.penalty}</span>`;
  html += '</div>';
  html += `<div class="ordering-details">${result.details}</div>`;
