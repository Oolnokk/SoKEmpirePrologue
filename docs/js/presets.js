export function initPresets(){
  ...
  const clone = (x)=> (x==null? (Array.isArray(x)? [] : {}) : JSON.parse(JSON.stringify(x)));
  C.presets = C.presets || {};

  C.presets.SLAM = {
    poses: clone(C.poses),
    durations: clone(C.durations),
    knockbackBase: 250,
    cancelWindow: 0.5
  };
  ...
  C.presets.KICK = {
    ...
    cancelWindow: 0.6,
    poses: {
      Stance: Object.assign(clone(C.poses?.Stance), { resetFlipsBefore: true }),
  ...
  (function defineKickAlt(){
    ...
    C.presets.KICK_ALT = {
      durations: clone(base.durations),
      poses: clone(base.poses),
      sequence: [
  ...
  C.presets.PUNCH = {
    ...
    cancelWindow: 0.7,
    poses: {
      Stance: clone(C.poses?.Stance),
      Windup: clone(C.poses?.Windup),
      Strike: clone(C.poses?.Strike),
      Recoil: clone(C.poses?.Recoil),
      Strike1: Object.assign(clone(C.poses?.Strike), { durMs:110, phase:'strike', torso:45, rShoulder:-35, rootMoveVel:{x:30,y:0}, impulseMag:90, impulseDirDeg:0, anim_events:[{ time:0.00, velocityX:260, velocityY:0, localVel:true }] }),
      Strike2: Object.assign(clone(C.poses?.Strike), { durMs:130, phase:'strike', torso:50, rShoulder:-45, rootMoveVel:{x:40,y:0}, impulseMag:110, impulseDirDeg:0, anim_events:[{ time:0.00, velocityX:300, velocityY:0, localVel:true }] })
    },
    sequence: [
...
export function ensureAltSequenceUsesKickAlt(){
  ...
  for (let i=0; i<C.combo.altSequence.length; i++){ if (C.combo.altSequence[i] === 'KICK'){ C.combo.altSequence[i] = 'KICK_ALT'; } }
}
