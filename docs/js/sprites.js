...
(() => {
  ...
  // Compute a point offset along a direction by percent units (matches HTML demo)
  function withAX(x, y, ang, axay) {
    if (!axay) return [x, y];
    const ax = +axay.ax || 0; // % in "xformUnits: percent"
    const ay = +axay.ay || 0;
    // Percent of segment length, but in drawBoneSprite we use image-local,
    // so here we just project ax, ay along angle and its normal.
  ...
  function getSpriteImage(key) {
    // window.SPRITES.cache[key] should exist when loaded
  ...
  window.drawBoneSprite = function drawBoneSprite(x, y, segLen, baseAng, partKey, wfOverride) {
    ...
    // 2) angle = bone direction + per-part rot (this mirrors the HTML)
    //    NOTE: if you flip horizontally at a higher level, keep the handedness consistent there.
    ...
    // 5) local scale (mirroring handled by the caller’s local flip blocks in your HTML)
    ctx.scale(sx, sy);

    // 6) optional width scaling to stretch/compress across segLen
    //    In your HTML, widthFactor multiplies the image width; it’s passed from caller.
  ...
  // ====== Convenience wrappers for torso/head if repo uses them directly ======
  // (These keep pivot semantics explicit; optional to use.)
  ...
  // If prior code expects sprites.torso but config renamed it, alias once at load time.
