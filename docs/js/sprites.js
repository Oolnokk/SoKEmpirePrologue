// sprites.js — v19 sprite parenting + local flip when fighter faces left
(function(){
  ...
  function angle(a,b){ return Math.atan2(b[0]-a[0], -(b[1]-a[1])); }
  ...
  function drawFighterSprites(ctx, C, G){
    ...
    for (const key of DRAW_ORDER){
      const img = imgs[key]; const bone = rig[key]; if (!bone) continue;
      const styleKey = mapStyleKey(key);
      drawBoneSprite(ctx, img, bone, styleKey, style, flip);
    }
  }
  ...
  console.log('[sprites] ready (v19 parenting + local flip)');
})();
