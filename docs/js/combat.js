// combat.js — set facing like v19 (0 = right, π = left) and move
...
function makeCombat(G, C){
  ...
  function handleInputAttacks(){
    ...
    if (taps.button1Tap){ const s = getKickQuick(C); if (s) startSequence(s, 'KICK'); }
    else if (taps.button2Tap){ const s = getHeavySlam(C); if (s) startSequence(s, 'SLAM'); }
  }
  ...
  function tick(dt){
    const p = P(); if (p){
      ...
      if (I.left && !I.right){ p.vel.x -= ax*dt; p.facingRad = Math.PI; }
      else if (I.right && !I.left){ p.vel.x += ax*dt; p.facingRad = 0; }
